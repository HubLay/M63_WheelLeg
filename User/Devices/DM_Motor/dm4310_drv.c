...
void clear_motor_state(hcan_t* hcan, uint16_t motor_id, uint16_t mode_id)
{
	uint8_t data[8];
	uint16_t id = motor_id + mode_id;
...
void mit_ctrl2(hcan_t* hcan, uint16_t motor_id, float pos, float vel,float kp, float kd, float torq)
{
	...
	int16_t id = 0x200;
	tor_tmp = torq * 3300;
	16384.0f / (MOTOR_TORQUE_To_CURRENT * 20.0f);
if(hcan==&hfdcan2)
{
	data[0] = 0;
	data[1] = 0;
	data[2] = 0;
	data[3] = 0;
	data[4] = (uint8_t)((tor_tmp >> 8) & 0x00FF);
	data[5] = (uint8_t)(tor_tmp & 0x00FF);
	data[6] = 0;
	data[7] = 0;
}else if(hcan==&hfdcan1)
{
	...
	data[4] =0;
	data[5] =0;
	data[6] = (uint8_t)((tor_tmp >> 8) & 0x00FF);
	data[7] = (uint8_t)(tor_tmp & 0x00FF);
