...
#define REFEREE_DMA_TX_QUEUE_DEPTH 80
...
struct_DwtTime ui_dwt_clpt;

void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart == &huart10) {
			ui_dwt_clpt.dt = DWT_GetDeltaT(&ui_dwt_clpt.cnt_last);
        // referee_dma_busy = 0;
        // Referee_DMA_Dequeue();
        // Referee_DMA_StartNext();
    }
...
void MiniPC_Status_Updata(uint8_t __Init_Cnt){

	static uint8_t MiniPC_Status_Name[] = "MiniPC";	
	uint8_t tmp_color = White;
	graphic_data_struct_t *P_graphic_data;
	...
	int op_type = (__Init_Cnt == 0) ? Op_Change : Op_Add;

	P_graphic_data = Rectangle_Draw(0, op_type, 510, 210, 510 + 900, 210 + 660, 2, tmp_color, MiniPC_Status_Name);
	memcpy(data_pack, (uint8_t *)P_graphic_data, DRAWING_PACK);
	Send_UIPack(Drawing_Graphic1_ID, JudgeReceiveData.robot_id, JudgeReceiveData.robot_id + 0x100, data_pack, DRAWING_PACK);
}

void Leg_Length_Updata(uint8_t __Init_Cnt){
	static uint8_t Leg_Length_Name[] = "Leg_Length";	

	graphic_data_struct_t *P_graphic_data;

	int op_type = (__Init_Cnt == 0) ? Op_Change : Op_Add;

	P_graphic_data = FloatData_Draw(0, op_type, 1625, 600, JudgeReceiveData.Leg_Length, 25, 1, 2, Cyan, Leg_Length_Name);
...
void SCapLine_Init(void)
{
    ...
    static uint8_t BarFrameName[] = "SCF";
		static uint8_t BarName[] = "SCB";
    ...
    uint16_t height = 300;
		...
		P_graphic_data = Rectangle_Draw(1,Op_Add,x,y,x + width,y + height,2,White,BarFrameName);

		memcpy(data_pack, (uint8_t *)P_graphic_data, DRAWING_PACK);

		Send_UIPack(Drawing_Graphic1_ID,JudgeReceiveData.robot_id,JudgeReceiveData.robot_id + 0x100,data_pack,DRAWING_PACK);

		P_graphic_data = Line_Draw(1,Op_Add,x,y,x + 0,y,14,Green,BarName);
...
void SCapLine_Change(void)
{
    ...
    uint16_t max_len = 300;

    uint16_t bar_len;

    uint8_t color;

    voltage = JudgeReceiveData.Supercap_Voltage;

    percent = voltage / 100.0f;
    ...
    bar_len = (uint16_t)(max_len * percent);
    ...
    P_graphic_data = Line_Draw(1,Op_Change,x,y,x,y + bar_len,17,color,BarName);
...
void ChassisLine_Change(float theta, uint8_t Init_Cnt)
{
	...
	float tmp_ref_rad = (int16_t)(Reference_Rad * 57.3f) % 360;

	theta = (int16_t)theta % 360;
	theta = -(-theta + tmp_ref_rad);

	// 计算圆弧的起始和终止角度
	// 随着夹角变化，圆弧整体旋转

	start_angle = (uint16_t)(-20 + theta) % 360;
	end_angle = (uint16_t)(20 + theta) % 360;
	...
	case 2: // Chassis_Control_Type_SPIN
		...
		break;
	case 3: // Chassis_Control_Type_JUMP_1
		...
		break;
	case 5:	// Chassis_Control_Type_Reserve
		P_graphic_data = Arc_Draw(0, optype, SCREEN_LENGTH * 0.5 + 632 + x_bias, SCREEN_WIDTH * 0.5 + 184 + y_bias, 0, 360, radius, radius, 15, Pink, ChassisLineName);
		break;
	default:			//错误状态
		P_graphic_data = Arc_Draw(0, optype, SCREEN_LENGTH * 0.5 + 632 + x_bias, SCREEN_WIDTH * 0.5 + 184 + y_bias, 0, 360, radius, radius, 15, Black, ChassisLineName);
		break;
...
void GIMLine_Change(uint8_t Init_Cnt)
{
	...
	static uint8_t GIMLineName2[] = "GL6";
	static uint8_t optype;	
	graphic_data_struct_t *P_graphic_data;
	...
	case 1: // Gimbal_Control_Type_NORMAL
		...
		memcpy(data_pack, (uint8_t *)P_graphic_data, DRAWING_PACK);
		Send_UIPack(Drawing_Graphic1_ID, JudgeReceiveData.robot_id, JudgeReceiveData.robot_id + 0x100, data_pack, DRAWING_PACK);
		P_graphic_data = Arc_Draw(0, optype, 960, 540, 170, 180, 300, 260, 10, White, GIMLineName2);
		memcpy(data_pack, (uint8_t *)P_graphic_data, DRAWING_PACK);
		Send_UIPack(Drawing_Graphic1_ID, JudgeReceiveData.robot_id, JudgeReceiveData.robot_id + 0x100, data_pack, DRAWING_PACK);
		break;
	case 2: // Gimbal_Control_Type_MINIPC
		...
		memcpy(data_pack, (uint8_t *)P_graphic_data, DRAWING_PACK);
		Send_UIPack(Drawing_Graphic1_ID, JudgeReceiveData.robot_id, JudgeReceiveData.robot_id + 0x100, data_pack, DRAWING_PACK);
		P_graphic_data = Arc_Draw(0, optype, 960, 540, 180, 190, 300, 260, 10, White, GIMLineName2);
		memcpy(data_pack, (uint8_t *)P_graphic_data, DRAWING_PACK);
		Send_UIPack(Drawing_Graphic1_ID, JudgeReceiveData.robot_id, JudgeReceiveData.robot_id + 0x100, data_pack, DRAWING_PACK);
		break;
	default:
...
void GraphicSendtask(void)
{
	...
	if (Init_Cnt > 0)
	{
		...
		MiniPC_Status_Updata(Init_Cnt);
		Leg_Length_Updata(Init_Cnt);
	...
	case UI_STATE_VALUE_UPDATE:
		...
		if(fabsf(Last_JudgeReceiveData.Leg_Length - JudgeReceiveData.Leg_Length) >= 0.01){
			Leg_Length_Updata(0);
			Last_JudgeReceiveData.Leg_Length = JudgeReceiveData.Leg_Length;
		}
