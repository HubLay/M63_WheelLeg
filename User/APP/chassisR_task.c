...
//	}
  ...
  chassis->turn_T=Turn_Pid.Kp*(chassis->turn_set-chassis->total_yaw)-Turn_Pid.Kd*ins->Gyro[2];//这样计算更稳一点
	...
	mySaturate(&chassis->wheel_motor[0].wheel_T,-2.2f,2.2f);	
	
	vmcr->Tp=vmcr->Tp+chassis->leg_tp;//髋关节输出力矩
	//vmcr->Tp=0;
	vmcr->F0=60.0f/arm_cos_f32(vmcr->theta)+PID_Calc(leg,vmcr->L0,chassis->leg_set);//前馈+pd
...
//	 
	...
	//额定扭矩
  mySaturate(&vmcr->torque_set[1],-15.0f,15.0f);	
