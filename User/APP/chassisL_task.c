...
//R=[90 0;0 4]; 
float LQR_K_L[12]={ 
 -11.5404 ,  -1.0211  , -1.3386,   -1.7482,    3.5008 ,   0.6718,
    6.7446 ,   0.5966 ,   1.2295   , 1.5040 ,   9.0135  ,  1.2184}
;
...
//	chassis->wheel_motor[1].wheel_T=0;
	...
	vmcl->Tp=(LQR_K[6]*(vmcl->theta-0.0f)
				...
				T_out[2]=LQR_K[2]*(chassis->x_set-chassis->x_filter);
	...
	mySaturate(&chassis->wheel_motor[1].wheel_T,-2.2f,2.2f);	
	
	vmcl->Tp=vmcl->Tp+chassis->leg_tp;//髋关节输出力矩
//	vmcl->Tp=0;
	vmcl->F0=60.0f/arm_cos_f32(vmcl->theta)+PID_Calc(leg,vmcl->L0,chassis->leg_set);//前馈+pd
